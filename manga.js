export default [
  {
    id: 1,
    name: "One Piece",
    image:
      "https://honeysanime.com/wp-content/uploads/2018/12/ONE-PIECE-Wallpaper-500x500.jpg",
    anotherName: ["Đảo hải tặc"],
    status: "Đang phát hành",
    author: "Eiichiro Oda",
    categories: [
      "HOT NHẤT",
      "SIÊU NĂNG LỰC",
      "ĐANG CẬP NHẬT",
      "MỚI NHẤT",
      "HÀNH ĐỘNG",
      "HÀI HƯỚC",
    ],
    types: ["Action", "Adventure", "Comedy", "Drama", "Fantascy", "Shounen"],
    description: `One Piece xoay quanh 1 nhóm cướp biển được gọi là Băng Hải tặc Mũ Rơm - Straw Hat Pirates - được thành lập và lãnh đạo bởi thuyền trưởng Monkey D. Luffy. Cậu bé Luffy có ước mơ tìm thấy kho báu vĩ đại nhất, One Piece, của Vua Hải Tặc đời trước Gold D. Roger và trở thành Vua Hải Tặc đời kế tiếp. Ở Việt Nam hiện nay, truyện đang được Nhà xuất bản Thanh Hóa xuất bản nhưng không có bản quyền, với tên gọi là Đảo Hải tặc Cốt truyện: Monkey D. Luffy, 1 cậu bé rất thích hải tặc có ước mơ tìm được kho báu One Piece và trở thành Vua hải tặc - Pirate King. Lúc nhỏ, Luffy tình cờ ăn phải trái quỉ (Devil Fruit) Gomu Gomu, nó cho cơ thể cậu khả năng co dãn đàn hồi như cao su nhưng đổi lại cậu sẽ không bao giờ biết bơi. Sau đó Luffy lại được Shank cứu thoát tuy nhiên ông ta bị mất 1 cánh tay. Sau đấy Shank chia tay Luffy và để lại cho cậu cái mũ rơm (Straw Hat) và nói rằng: "Sau này bao giờ thành cướp biển hãy gặp ta và trả lại nó". Chính lời nói này đã thúc đầy Luffy trở thành 1 cướp biển thật sự.`,
  },
  {
    id: 2,
    name: "Gintanma",
    image:
      "https://www.animenewsnetwork.com/hotlink/images/encyc/A15223-2335294335.1364400831.jpg",
    anotherName: ["Linh hồn bạc"],
    status: "Đã hoàn thành",
    author: "Hideaki Sorachi",
    categories: [
      "HOT NHẤT",
      "SIÊU NĂNG LỰC",
      "ĐÃ FULL",
      "HÀNH ĐỘNG",
      "HÀI HƯỚC",
    ],
    types: [
      "Action",
      "Adventure",
      "Comedy",
      "Parody",
      "Shounen",
      "Drama",
      "Tragedy",
    ],
    description: `Câu chuyện xoay xung quanh cuộc sống hằng ngày của 3 nhân vật chính, Sakata Gintoki, một Samurai hết thời, bị bệnh tiểu đường nặng, Shinpachi Shimura, kẻ kế thừa của một võ đường xập xệ do chẳng còn ai đến luyện võ trong thời đại này và Kagura, một Amanto với sức mạnh phi thường, tham ăn trong thân hình nhỏ bé của 1 cô bé 14 tuổi. Cả 3 phải làm đủ mọi việc (hầu hết là phá hoại chứ chẳng làm gì nghiêm túc) để trả tiền nhà hằng tháng.`,
  },
  {
    id: 3,
    name: "Dragon Ball Z",
    image:
      "https://vignette.wikia.nocookie.net/dragonball/images/a/aa/Son_Goku_Song.jpg/revision/latest?cb=20160818153616&path-prefix=es",
    anotherName: ["7 (Bảy) viên ngọc rồng"],
    status: "Đã hoàn thành",
    author: "Toriyama Akira",
    categories: [
      "HOT NHẤT",
      "SIÊU NĂNG LỰC",
      "ĐÃ FULL",
      "HÀNH ĐỘNG",
      "HÀI HƯỚC",
    ],
    types: [
      "Action",
      "Adventure",
      "Comedy",
      "Drama",
      "Fantacy",
      "Shounen",
      "Supernatural",
    ],
    description: `Một cậu bé có đuôi khỉ tên là Goku được tìm thấy bởi một ông lão sống một mình trong rừng, ông lão xem đứa bé như là cháu của mình. Một ngày nọ Goku tình cờ gặp một cô gái tên là Bulma trên đường đi bắt cá về, Goku và Bulma đã cùng nhau truy tìm bảy viên ngọc rồng. Bảy viên ngọc rồng chứa đựng một bí mật có thể triệu hồi một con rồng và ban điều ước cho ai sở hữu chúng. Trên cuộc hành trình dài đi tìm bảy viên ngọc rồng, họ gặp những người bạn và những đấu sĩ huyền thoại cũng như nhiều ác quỷ. Hãy cùng đón xem hành trình của họ nhé`,
  },
  {
    id: 4,
    name: "Naruto",
    image: "https://i1.sndcdn.com/artworks-000199811034-rw0tnm-t500x500.jpg",
    anotherName: ["Cáo chín đuôi"],
    status: "Đã hoàn thành",
    author: "Kishimoto Mashshi",
    categories: [
      "HOT NHẤT",
      "SIÊU NĂNG LỰC",
      "ĐÃ FULL",
      "HÀNH ĐỘNG",
      "HÀI HƯỚC",
    ],

    types: [
      "Action",
      "Adventure",
      "Comedy",
      "Drama",
      "Fantacy",
      "Shounen",
      "Supernatural",
    ],
    description: `Naruto là một cậu bé có mơ ước trở thành hokage của làng Konoha,được Hokage phong ấn trong người một trong 9 quái vật của thể giới : Cửu vĩ Hồ ly. Vì cho cậu là một con quái vật, ko ai dám chơi với cậu!& Vì muốn được thừa nhận nên rất phá phách.Khi tốt nghiệp trướng ninja, lần đầu tiên cậu đã được thừa nhận và có một người bạn thân là Sasuke`,
  },
  {
    id: 5,
    name: "Bleach",
    image:
      "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/5b60d491-f905-4356-9e4a-d9a8f1beb5a3/d5byvxq-21839a0a-333b-4bef-ac4d-8c073133991d.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1cm46YXBwOiIsImlzcyI6InVybjphcHA6Iiwib2JqIjpbW3sicGF0aCI6IlwvZlwvNWI2MGQ0OTEtZjkwNS00MzU2LTllNGEtZDlhOGYxYmViNWEzXC9kNWJ5dnhxLTIxODM5YTBhLTMzM2ItNGJlZi1hYzRkLThjMDczMTMzOTkxZC5qcGcifV1dLCJhdWQiOlsidXJuOnNlcnZpY2U6ZmlsZS5kb3dubG9hZCJdfQ.jK7-KIuZuolQC8rZt3NLM_hHyro0I5_k5ZQnwKIr99A",
    anotherName: ["Sứ giả thần chết"],
    status: "Đã hoàn thành",
    author: "Kubo Tite",
    categories: ["HOT NHẤT", "SIÊU NĂNG LỰC", "HÀNH ĐỘNG", "HÀI HƯỚC"],
    types: [
      "Action",
      "Adventure",
      "Comedy",
      "Drama",
      "Fantacy",
      "Shounen",
      "Supernatural",
    ],
    description: `Ichigo Kurosaki có khả năng nhìn thấy những hồn ma. Cuộc sống của cậu thay đổi khi cậu gặp Rukia Kuchiki, một Thần Chết và là thành viên của Âm Giới. 
    `,
  },
  {
    id: 6,
    name: "Doraemon",
    image:
      "https://i.pinimg.com/originals/35/b6/fa/35b6fa5f83b79ebb4d5a39785fe5285e.jpg",
    anotherName: ["Chú mèo máy đến từ tương lai"],
    status: "Đang phát hành",
    athor: "Fujiko F.Fujio",
    categories: ["HOT NHẤT", "VIỄN TƯỞNG", "ĐANG CẬP NHẬT", "HÀI HƯỚC"],
    types: ["Comedy", "Slice of life", "Fantascy"],
    description: `Đôrêmon là một chú mèo máy được Nôbita, cháu ba đời của Nôbita gửi về quá khứ cho ông mình để giúp đỡ Nôbita tiến bộ, tức là cũng sẽ cải thiện hoàn cảnh của con cháu Nôbita sau này. Còn ở hiện tại, Nôbita là một cậu bé luôn thất bại ở trường học, và sau đó là thất bại trong công việc, đẩy gia đình và con cháu sau này vào cảnh nợ nần. Các câu chuyện trong Đôrêmon thường có một công thức chung, đó là xoay quanh những rắc rối hay xảy ra với cậu bé Nôbita lớp bốn, nhân vật chính thứ nhì của bộ truyện. Đôrêmon có một chiếc túi thần kỳ trước bụng với đủ loại bảo bối của tương lai. Cốt truyện thường gặp nhất sẽ là Nôbita trở về nhà khóc lóc với những rắc rối mà cậu gặp phải ở trường hoặc với bạn bè. Sau khi bị cậu bé van nài hoặc thúc giục, Đôrêmon sẽ đưa ra một bảo bối giúp Nôbita giải quyết những rắc rối của mình, hoặc là để trả đũa hay khoe khoang với bạn bè của cậu. Nôbita sẽ lại thường đi quá xa so với dự định ban đầu của Đôrêmon, thậm chí với những bảo bối mới cậu còn gặp rắc rối lớn hơn trước đó. Đôi khi những người bạn của Nôbita (thường là Xêkô hoặc Chaien) lại lấy trộm những bảo bối và sử dụng chúng không đúng mục đích. Tuy nhiên thường thì ở cuối mỗi câu chuyện, những ai sử dụng sai mục đích bảo bối sẽ phải chịu hậu quả do mình gây ra, và người đọc sẽ rút ra được bài học đạo đức từ đó.`,
  },
  {
    id: 7,
    name: "Shingeki No Kyojin",
    image:
      "https://img.discogs.com/nCs8Q2Jokml0rQdxrXf5UjXELwY=/fit-in/600x600/filters:strip_icc():format(jpeg):mode_rgb():quality(90)/discogs-images/R-4711902-1373077114-6939.jpeg.jpg",
    anotherName: ["Attack on Titan", "Đại chiến người khổng lồ"],
    status: "Đang tiến hành",
    categories: ["HOT NHẤT", "KINH DỊ", "ĐANG CẬP NHẬT"],
    types: ["Action", "Trangedy", "Drama", "Fantacy", "Shounen", "Horror"],
    description: `Hơn 100 năm trước, giống người khổng lồ Titan đã tấn công và đẩy loài người tới bờ vực tuyệt chủng. Những con người sống sót tụ tập lại, xây bao quanh mình 1 tòa thành 3 lớp kiên cố và tự nhốt mình bên trong để trốn tránh những cuộc tấn công của người khổng lồ. Họ tìm mọi cách để tiêu diệt người khổng lồ nhưng không thành công. Và sau 1 thế kỉ hòa bình, giống khổng lồ đã xuất hiện trở lại, một lần nữa đe dọa sự tồn vong của con người....
    Elen và cô em gái Mikasa phải chứng kiến một cảnh tượng cực kinh khủng - mẹ của mình bị ăn thịt ngay trước mắt. Elen thề rằng cậu sẽ giết tất cả những tên khổng lồ mà cậu gặp...`,
  },
];
